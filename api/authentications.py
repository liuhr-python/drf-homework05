...
class MyAuth(BaseAuthentication):
    ...
    def authenticate(self, request):
        ...
        if not (len(auth_list) == 2 and auth_list[0].lower() == "asd"):
            raise exceptions.AuthenticationFailed("认证信息有误，认证失败")

        # 如果认证成功 则解析用户  规定认证信息必须为drf.admin.123
        if auth_list[1] != "drf.liuhr-python.123":
            raise exceptions.AuthenticationFailed("用户信息校验失败")

        # 最后校验数据库是否存在此用户
        user = User.objects.filter(username="liuhr-python").first()
